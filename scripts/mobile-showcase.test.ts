...
it("expands both appearances into independent upload-ready directories", () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      { appearance: "light", directory: "/captures/apple/iphone-test/light/zerops" },
      { appearance: "dark", directory: "/captures/apple/iphone-test/dark/zerops" },
    ],
  );
});
...
it("defaults every device to the app's own palette", () => {
  assert.equal(DEFAULT_SHOWCASE_THEME, "zerops");
  assert.equal(
