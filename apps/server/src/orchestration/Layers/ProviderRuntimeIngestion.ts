...
const make = Effect.gen(function* () {
  ...
  /** A projected message of this thread, or undefined. */
  const getThreadMessageById = (threadId: ThreadId, messageId: MessageId) =>
    projectionSnapshotQuery.getTurnStartMessage({ threadId, messageId }).pipe(
      Effect.map((entry) => Option.getOrUndefined(Option.map(entry, (value) => value.message))),
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (reasoningDelta && reasoningDelta.delta.length > 0 && reasoningTurnId) {
        ...
        // Reasoning is never delivered token by token, even with legacy token
        // streaming on: the block is collapsed by default, so a command, an
        // event-store write and a fan-out per token would buy nothing. Traces
        // are longer than the answers they precede.
        const spillChunk = yield* appendBufferedAssistantText(reasoningMessageId, delta);
