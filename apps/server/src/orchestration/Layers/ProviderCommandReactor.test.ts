...
describe("ProviderCommandReactor", () => {
  ...
  // Upstream also stops a session while a replayed send is still in flight. The
  // fork's ungated path sends inline, so that scenario cannot arise there.
  effectIt.effect.each(["resume", "stop before resume"])(
  ...
  it("pins the first user message when regeneration context is truncated", async () => {
    ...
    expect(message).toContain(
      `USER:\nReview subagent monitoring risks. ${"Opening context. ".repeat(5)}`,
    );
