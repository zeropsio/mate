...
const make = Effect.gen(function* () {
  ...
  const resumeTurnsAfterCompaction = Effect.fn("resumeTurnsAfterCompaction")(function* (
    ...
    while (queued.length > 0 && turnsAfterCompaction.get(threadId) === queued) {
      ...
      // The durable user message the queued start carried; upstream reads it
      // through getTurnStartMessage (17490c0a0), which the fork does not have yet.
      const turnStart = yield* projectionSnapshotQuery
        ...
        .pipe(
          ...
          Effect.map(Option.map((message) => ({ message }))),
        );
  ...
  const ensureThreadWorktree = Effect.fnUntraced(function* (thread: {
    ...
    const submodules = yield* serverSettingsService.getSettings.pipe(
  ...
  const buildSendTurnRequestForThread = Effect.fnUntraced(function* (input: {
    ...
    /** Facts the agent has not been told — placed in front of the text, never stored. */
    readonly agentNotes?: ReadonlyArray<string>;
  ...
  const maybeGenerateThreadTitleForFirstTurn = Effect.fn("maybeGenerateThreadTitleForFirstTurn")(
    function* (input: {
      ...
      const attachments = input.attachments ?? [];
      yield* Effect.gen(function* () {
        const titled = yield* resolveThreadShell(input.threadId);
        if (!titled) return;
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    // The send below settles `sent` from here on, so drop the entry the post-processing hook uses.
