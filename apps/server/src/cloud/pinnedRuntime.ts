...
/**
 * A pinned runtime is an exact `zerops-code@<version>` npm-installed into
 ...
 * switching over, never `npx zerops-code`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "zerops-code", "dist", "bin.mjs"),
...
/**
 * Installs `zerops-code@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "zerops-code", "dist", "bin.mjs"),
    sentinelPath: input.path.join(stagingDir, ".install-complete"),
  };

  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned zerops-code runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
