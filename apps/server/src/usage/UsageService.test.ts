...
const serviceLayers = (input: {
  ...
  readonly ratesDocument?: Record<string, unknown>;
}) =>
    ...
    Layer.provideMerge(
      Layer.succeed(
        ...
        HttpClient.make((request) =>
          Effect.sync(() => {
            input.onRatesFetch?.();
            // Without a rates document the rates are unparsable: every scan
            // retries the fetch, which makes the fetch count a boundary-level
            // observation of how many scans ran.
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.include(
            ...
            NodeFS.realpathSync(NodePath.join(configured, "projects")),
          );
          ...
          assert.include(
            ...
            NodeFS.realpathSync(NodePath.join(environmentHome, "projects")),
          );
        }).pipe(
          Effect.provide(
            serviceLayers({
          ...
          Effect.provideService(FileSystem.FileSystem, {
            ...
            exists: (path) =>
              fileSystem.exists(path).pipe(
                Effect.tap(() => {
                  // Transcript homes are scanned by their real path (macOS: /private/var).
                  if (path !== NodeFS.realpathSync(NodePath.join(home, "claude", "projects"))) {
                    return Effect.void;
                  }
                  homeProbes += 1;
