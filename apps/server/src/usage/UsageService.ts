...
export const make = Effect.gen(function* () {
  ...
  const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(function* (
    ...
    for (const driver of ["claudeAgent", "codex", "grok"] as const) {
      ...
      for (const instance of instances) {
        const environment = providerInstanceEnvironment(instance.environment, hostEnvironment);
  ...
  const collectDirs = Effect.fn("UsageService.collectDirs")(function* (
    ...
    const loginUnavailable =
      ...
      // Cursor's default macOS login lives in the Keychain, which this server never reads.
  ...
  const scanSummary = Effect.fn("UsageService.scanSummary")(function* (
    ...
    const [, scannedDirs] = yield* Effect.all(
      [ensureRates(), collectDirs(windowStartMs, settings, retentionCutoffMs)],
      { concurrency: 2 },
    );
