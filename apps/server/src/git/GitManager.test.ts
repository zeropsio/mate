  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(ghCalls).toContain(
          "pr list --head main --state all --limit 100 --json number,title,url,baseRefName,headRefName,state,mergedAt,updatedAt,isCrossRepository,headRepository,headRepositoryOwner",
        );
      }),
    20_000,
  );
