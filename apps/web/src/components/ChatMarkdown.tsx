...
export function hasMarkdownFilePrimaryAction(input: {
  ...
  return input.canOpenInEditor || input.canOpenInPanel;
}
...
function useChatMarkdownState({
  ...
  return { componentState, handleCopy, markdownUrlTransform };
}
...
const CHAT_MARKDOWN_COMPONENTS = {
  ...
  code: function MarkdownCode({ node, children, className, ...props }) {
    const { cwd, fileLinkChip, inlineCodeFileLinkMetaByText } = use(ChatMarkdownRendererContext);
  ...
  pre: function MarkdownPre({ node, children, ...props }) {
    const { diffThemeName, isStreaming, onRunShellCommand, resolvedTheme, text } = use(
