...
import type * as UsagePagePreferencesModule from "./usagePagePreferences";

// Preferences live in the signed-in account's storage, which binds to
// `window.localStorage` when the module loads.
let key: string;
let values: Map<string, string>;
let storage: Pick<Storage, "getItem" | "setItem" | "removeItem">;
let readUsagePagePreferences: typeof UsagePagePreferencesModule.readUsagePagePreferences;
let saveUsagePagePreferences: typeof UsagePagePreferencesModule.saveUsagePagePreferences;
let closeAccountLifetime: () => void;
...
beforeEach(async () => {
  ...
  vi.resetModules();
  const lifetime = await import("../../zerops/accountLifetime");
  lifetime.openAccountLifetime("usage-preferences-test");
  closeAccountLifetime = lifetime.closeAccountLifetime;
  key = lifetime.accountStorageKey("t3code:usage-page-preferences:v1")!;
  ({ readUsagePagePreferences, saveUsagePagePreferences } = await import("./usagePagePreferences"));
});
