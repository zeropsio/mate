...
function FailureAction(props: {
  readonly label: string;
  readonly onPress: () => void;
  readonly tone: "primary" | "secondary" | "text";
}) {
  return (
    <Pressable
      ...
      className={cn(
        "min-h-11 items-center justify-center rounded-full px-5 active:opacity-70",
        props.tone === "primary" ? "bg-primary" : props.tone === "secondary" ? "bg-subtle" : null,
      )}
    >
      <Text
        className={cn(
  ...
  return (
    ...
    >
      ...
      <View className="w-full max-w-xs gap-2">
        <FailureAction label="Try again" onPress={props.retry} tone="primary" />
        <FailureAction
          label="Copy details"
        ...
        />
        {props.exit ? (
          <FailureAction label={props.exit.label} onPress={props.exit.onPress} tone="text" />
        ) : null}
      </View>
    </ScrollView>
  );
