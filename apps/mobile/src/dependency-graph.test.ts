...
describe("mobile dependency graph", () => {
  ...
  it("keeps upward imports from state/lib/components/native into features at the ceiling", () => {
    ...
    const ceilings: ReadonlyArray<readonly [Layer, Layer, number, string]> = [
      // state -> features: the incoming-share store, the connection
      // controller hook, the terminal launch context, and the pending message
      // feed. (legacy-plan-mode was pure model logic and moved into state/.)
      ["state", "features", 4, "state must not add imports from features"],
      ...
      ["lib", "state", 5, "lib must not add imports from state"],
    ];
