...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/Zerops Mate.app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/Zerops Mate.app/Contents/Resources",
...
const makeElectronAppLayer = (calls: ElectronAppCalls) =>
  Layer.succeed(ElectronApp.ElectronApp, {
    ...
    name: Effect.succeed("Zerops Mate"),
...
const withIdentity = <A, E, R>(
  ...
  input: {
    ...
    readonly currentUserDataDirExists?: boolean;
              ...
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    (input.legacyPathExists === true && path.includes("T3 Code (Alpha)")) ||
                      (input.currentUserDataDirExists === true && path.endsWith("/t3code")),
                  ),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  ...
  it.effect("adopts the pre-rename userData path when the oldest legacy path is absent", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;
      ...
      { currentUserDataDirExists: true },
    ),
  );

  it.effect("falls back to the new userData path when no legacy path exists", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/mate");
      }),
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Zerops Mate (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Zerops Mate (Alpha)");
