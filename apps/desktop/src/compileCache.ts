...
// so later launches skip recompiling the large main-process bundle.
// Packaged builds only: boot.ts loads it before main.cjs. Dev launches main.cjs
  ...
  if (!process.env.APPIMAGE) {
    const cacheRoot =
      ...
      process.platform === "linux"
        ...
        : NodeOS.tmpdir();
    NodeModule.enableCompileCache(NodePath.join(cacheRoot, "zerops-mate", "compile-cache"));
  }
} catch {
