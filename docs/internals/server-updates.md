...
`zcp` selects a mate GitHub release, pins its digest, downloads the release tarball, verifies it, and
installs it in the project container. The platform supervises that installed server as `zerops@mate`.
...
A standalone operator downloads `zerops-mate-<version>.tgz` and `SHA256SUMS` from a
...
`mate service install` and `mate service update` register the already installed server entry point with
the host service manager. They never fetch a package. To update a background service, install the
new release tarball first and then run that release's `mate service update` command.
