...
| [`hosted-web-cd.md`](hosted-web-cd.md) | How the hosted web client reaches Zerops: the `deploy` vs `push` mechanics measured live, the cache/version/label defects, where the client is allowed to live, and one parked alternative | Facts stay; the parked design dies when the delivery route is settled                        |
