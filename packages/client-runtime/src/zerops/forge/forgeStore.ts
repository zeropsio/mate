...
export function makeForgeStore(ports: ForgeStorePorts): ForgeStore {
  ...
  /** The open list that names this pull request, when one is held. */
  const listOf = (entry: Entry): Entry | undefined => {
    if (entry.fact.kind !== "pull") return undefined;
    const { origin, owner, repo, number } = entry.fact;
    const list = entries.get(keyOf({ kind: "open-pulls", origin, owner, repo }));
    if (list === undefined) return undefined;
    return holds<"open-pulls">(list, (numbers) => numbers.includes(number)) ? list : undefined;
  };

  /** The most urgent demand on it, directly or through the open list that names it. */
  const rankOf = (entry: Entry): number | null => {
    ...
    return list === undefined || list.leases.size === 0 ? null : rankOf(list);
  };
  ...
  const admit = (entry: Entry, readOrdinal: number, at: Instant, outcome: Outcome): void => {
    ...
    if (fact.kind === "pull") {
      ...
      const pulls =
        fact.kind === "open-pulls" ? (outcome.value as ReadonlyArray<GiteaPullRequest>) : null;
      ...
      const now = ports.now();
      entry.pollAt = backstopAfter(entry, value, now.mono, now.wall);
      if (pulls !== null && fact.kind === "open-pulls") {
