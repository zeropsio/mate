...
describe("classifyZeropsCall — pending-unclassifiable (R4)", () => {
  it("a running zerops_workflow with no arguments yet is not renderable", () => {
    expect(classifyZeropsCall("zerops_workflow", {}, "inProgress")).toBe("pending-unclassifiable");
    expect(classifyZeropsCall("zerops_workflow", undefined, "inProgress")).toBe(
      "pending-unclassifiable",
    );
  });

  it("a running zerops_mount with no arguments yet is not renderable", () => {
    expect(classifyZeropsCall("zerops_mount", {}, "inProgress")).toBe("pending-unclassifiable");
  });

  it("an interrupted call that never received arguments renders as a generic row, not silently forever", () => {
    expect(classifyZeropsCall("zerops_workflow", {}, "interrupted")).toBe("generic");
    expect(classifyZeropsCall("zerops_mount", {}, "interrupted")).toBe("generic");
  });

  it("a completed zerops_workflow/zerops_mount with empty input is classified normally, not pending", () => {
    // Completed with `{}` never happens on real transcripts (arguments are
    // always present by completion), but the gate must not swallow it.
    expect(classifyZeropsCall("zerops_workflow", {}, "completed")).toBe("generic");
    expect(classifyZeropsCall("zerops_mount", {}, "completed")).toBe("card");
  });
});
