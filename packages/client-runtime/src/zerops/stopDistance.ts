...
import { PRODUCTION_DEPLOYING, PRODUCTION_NOTHING_LIVE, type GroupFlowStop } from "./groupFlow.ts";
import {
  deployedCommit,
  deployTone,
...
export function stopRowLine(input: {
  ...
  if (stop.state === "empty")
    return word("empty", input.tier === "production" ? PRODUCTION_NOTHING_LIVE : NOTHING_DEPLOYED);
